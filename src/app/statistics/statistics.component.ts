...
export class StatisticsComponent implements OnInit {
  ...
  const optionsDailySalesChart: any = {
      ...
      low: Math.min(...serie),
      high: Math.max(...serie)+Math.max(...serie)*0.01, // creative tim: we recommend you to set the high sa the biggest value + something for a better look
  ...
  ngOnInit() {
    ...
    this.loadDashboard();
    setInterval(() => this.loadDashboard(), 120000);
  }
  ...
  getChangeRatesGraph(table){
    this.niceHashService.getChangeRateStats(table).subscribe({
      next: value => {
        ...
        var serie = changeRates.result.map( row => row.value).map( str => Number(Number(str).toFixed(0))).reverse()
